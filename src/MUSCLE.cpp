...
List MUSCLE(NumericVector &Y, NumericVector &q, double b, bool test){
   ...
   double Q_max = fmax(-log(b),-log(1-b)); //maximum of f on [0,1]
   ...
   for (int i = 0; i< n; ++i) {
     ...
     for (int p = power ; p>= p_min; p--) {
       ...
       for (int k = 0; k <= i-l+1; k = k+ 2) {
         // Rcout << "k = " << k << "\n";
         idx = fmin(fmax(1,(int)ceil(l*b)),l);
     ...
     if(lb <= ub){
       ...
       // Rcout << "lb = " << lb << "\n";
       // Rcout << "ub = " << ub << "\n";
       C[i] = sum((Y[Range(0,i)]-mu[i])*(b-ifelse(Y[Range(0,i)]<mu[i],1.0,0.0)));
       Rcout << "C[i]" << i << "=" << C[i] << "\n";
       L[i] = 1;
       J[i] = 0;
     }else{
         ...
         for (int j=i-1; j>=jmin;j--) {
           if(J[j] == njmp){
             ...
             if(lb<=ub){
               ...
               aux_c = sum((Y[Range(j+1,i)]-aux_mu)*(b-ifelse(Y[Range(j+1,i)]<aux_mu,1.0,0.0)))+C[j];
               if(aux_c < C[i]){
                 C[i] = aux_c;
                 mu[i] = aux_mu;
                 L[i] = j+2;
...
//' idealization with MUSCLE
 ...
 List MMUSCLE(NumericVector &Y, NumericMatrix &q, NumericVector & b, bool test){
   ...
   for(int i = 0;i < num_beta; ++i){
     Q_max[i] = fmax(-log(b[i]),-log(1-b[i]));
     l_min[i] = floor(1/fmin(b[i],1-b[i]));
     ...
     for(int idx_beta = 0; idx_beta < num_beta; idx_beta++){
       for(int j = 0; j<n; j++){
         if(j==i){
           ...
           Q_tilde(idx_beta,j) = pow(penfsCC(j,l,q(idx_beta,j)),2)/(2*l);
   ...
   for (int i = 0; i< n; ++i) {
     ...
     for (int p = p_min; p <= power; p++) {
       ...
       for (int k = 0; k <= i-l+1; k = k+ 2) {
         ...
         for(int idx_beta = 0; idx_beta < num_beta; idx_beta++){
           ...
           idx = fmin(fmax(1,(int)ceil(l*b[idx_beta])),l);
           ...
           if(l >= l_min[idx_beta]){
             ...
             q_tilde = pow(penfsCC(i,l,q(idx_beta,i)),2)/(2*l);
             // Rcout << "q_tilde = " << q_tilde << "\n";
             // Rcout << "Q_max[idx_beta] =" << Q_max[idx_beta] << "\n";
             if(q_tilde<=Q_max[idx_beta]-1e-6){
               lq[idx_beta] = x1(q_tilde,b[idx_beta]);
               ...
               if(b[idx_beta]==0.5){
                 ...
                 uq[idx_beta] = x2(q_tilde,b[idx_beta]);
     ...
     if(no_empty == true){
       for(int idx_beta = 0; idx_beta < num_beta; idx_beta++){
         ...
         C(idx_beta,i) = sum((Y[Range(0,i)]-mu(idx_beta,i))*(b[idx_beta]-ifelse(Y[Range(0,i)]<mu(idx_beta,i),1.0,0.0)));
         ...
         for (int j=i-1; j>=jmin;j--) {
           ...
           if(J[j] == njmp){
               ...
               for (int k=j; k<= i-l; k = k+2) {
                 ...
                 for(int idx_beta = 0; idx_beta < num_beta; idx_beta++){
                   ...
                   if(l >= l_min[idx_beta]){
                     ...
                     q_tilde = pow(penfsCC(i-j-1,l,q(idx_beta,i-j-1)),2)/(2*l);
                     if(q_tilde<=Q_max[idx_beta]-1e-6){
                       lq[idx_beta] = x1(q_tilde,b[idx_beta]);
                       lq_idx[idx_beta] = fmin(fmax(1,(int)ceil(l*lq[idx_beta])),l);
                       if(b[idx_beta]==0.5){
             ...
             if(no_empty == true){
               ...
               aux_c = 0;
               for(int idx_beta = 0; idx_beta < num_beta; idx_beta++){
                  aux_c = aux_c + sum((Y[Range(j+1,i)]-aux_mu[idx_beta])*(b[idx_beta]-ifelse(Y[Range(j+1,i)]<aux_mu[idx_beta],1.0,0.0)));
               }
               ...
               for(int idx_beta = 0; idx_beta < num_beta; idx_beta++){
                 ...
                 if(q_tilde<=Q_max[idx_beta]-1e-6){
                   lq_max_i = x1(q_tilde,b[idx_beta]);
                 ...
                 if(q_tilde<=Q_max[idx_beta]-1e-6){
                   uq_max_i = x2(q_tilde,b[idx_beta]);
               ...
               for(int idx_beta = 0; idx_beta < num_beta; idx_beta++){
                 q_tilde = Q_tilde_max[llen[idx_beta]-1];
                 if(q_tilde<=Q_max[idx_beta]-1e-6){
                   lq_max_j = x1(q_tilde,b[idx_beta]);
                 ...
                 q_tilde = Q_tilde_max[ulen[idx_beta]-1];
                 if(q_tilde<=Q_max[idx_beta]-1e-6){
                   uq_max_j = x2(q_tilde,b[idx_beta]);
  ...
  for (int r = 0; r<R;r++) {
    ...
    for (int I = 0; I < n; I++) {
      ...
      for (int l = 1; l<= (I+1); l++) {
        ...
        um[l-1] = std::max(um[l-1],sqrt(2*l*ff(X_bar,b,0)));
      }
...
// [[Rcpp::export(.simulQuantile_DMUSCLE2)]]
